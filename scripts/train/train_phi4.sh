...
VT_VERSION=/HNA/model/siglip2-so400m-patch14-384 #vision tower path in huggingface
...
bash scripts/train/finetune_down_moelora.sh "$REASON_DATA_PATH2" "$REASON_IMAGE_PATH2" "$LLM_VERSION" "$VT_VERSION" "$VT_VERSION2" "$CN_VERSION" "$CONV_VERSION" "$VERSION" "$TRAIN_RECIPE" "$MODEL_MAX_LENGTH"

...
deepspeed --include localhost:0,1,2,3 --master_port 29501 tinyllava/train/custom_finetune.py \
    ...
    --pretrained_model_path /HNA/best/tiny-llava-Phi-4-mini-instruct-siglip2-so400m-patch14-384-base-finetune-down-share-e6 \
    --output_dir /HNA/outputs/48/tiny-llava-${LLM_VARIANT}-${VT_VARIANT}-${VERSION}-finetune-down-reason-48-5 \
    --task_num 2 \
    ...
    --run_name tiny-llava-${LLM_VARIANT}-${VT_VARIANT}-${VERSION}-finetune_down_reason
